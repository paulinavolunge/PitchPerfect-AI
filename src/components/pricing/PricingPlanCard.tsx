...
const PricingPlanCard: React.FC<PricingPlanCardProps> = ({
  ...
  return (
    <Card className={`border-2 ${isPopular ? 'border-brand-green shadow-lg' : 'shadow-sm'}`}>
        ...
        {type !== 'enterprise' ? (
          <>
            <div className="flex justify-between items-center">
              ...
              {isCurrentPlan && (
                <span className="bg-brand-green/20 text-brand-green text-xs font-medium px-2 py-1 rounded">CURRENT</span>
              )}
            </div>
            ...
            <div className="mt-4">
              {price}
              {priceDescription && (
                <span className="text-gray-500 ml-2">{priceDescription}</span>
              )}
            </div>
          </>
        ) : (
          <>
            ...
            <div className="mt-4 mb-2">
              <Tabs 
                value={enterpriseProps?.enterpriseSize} 
                onValueChange={(v) => enterpriseProps?.setEnterpriseSize(v as "small" | "medium" | "large")}
                className="w-full"
              >
