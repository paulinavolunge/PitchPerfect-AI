import { EMAIL_CONFIG } from '../../supabase/functions/_shared/prospect/scenarios/email';
...
describe('GamifiedRoleplay Phase 2A reliability', () => {
  ...
  it.each([
    ...
    if ((scenario === 'Budget' || scenario === 'Send Me an Email')) vi.mocked(fetch).mockImplementation(async (_url, init) => {
      const payload=JSON.parse(String(init?.body));
      ...
      return apiResult({...success(payload.turnId, scenario === 'Budget' ? OPENING : EMAIL_CONFIG.opening),sessionId:payload.sessionId,prospectState:scenario === 'Budget' ? initialState() : EMAIL_CONFIG.initialState()});
    });
    render(<GamifiedRoleplay />);
    fireEvent.click(screen.getByText(scenario));
    ...
    expect(screen.getByText((scenario === 'Budget' || scenario === 'Send Me an Email') ? 'Round 0' : 'Round 1')).toBeTruthy();
    ...
    vi.mocked(fetch).mockImplementation(async (_url, init) => {
      ...
      return apiResult({ ...success(payload.turnId, 'Let us discuss the business case.'), sessionId: payload.sessionId, ...((scenario === 'Budget' || scenario === 'Send Me an Email') ? {prospectState:{...initialState(),state:'GUARDED',turnCount:1,patience:78}} : {}) });
    });
    ...
    expect(screen.getByText((scenario === 'Budget' || scenario === 'Send Me an Email') ? 'Round 1' : 'Round 2')).toBeTruthy();
